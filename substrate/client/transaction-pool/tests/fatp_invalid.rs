...
fn fatp_transactions_purging_invalid_on_finalization_works() {
	...
	assert_watcher_stream!(watcher1, [TransactionStatus::Ready, TransactionStatus::Invalid(TransactionValidityError::Invalid(InvalidTransaction::Custom("Some reason".into())))]);
	assert_watcher_stream!(watcher2, [TransactionStatus::Ready, TransactionStatus::Invalid(TransactionValidityError::Invalid(InvalidTransaction::Custom("Some reason".into())))]);
	assert_watcher_stream!(watcher3, [TransactionStatus::Ready, TransactionStatus::Invalid(TransactionValidityError::Invalid(InvalidTransaction::Custom("Some reason".into())))]);
}
...
fn fatp_transactions_purging_invalid_on_finalization_works2() {
	...
	assert_watcher_stream!(watcher, [TransactionStatus::Ready, TransactionStatus::Invalid(TransactionValidityError::Invalid(InvalidTransaction::Custom("Some reason".into())))]);
...
fn fatp_watcher_invalid_many_revalidation() {
	...
	assert_watcher_stream!(xt3_watcher, [TransactionStatus::Ready, TransactionStatus::Invalid(TransactionValidityError::Invalid(InvalidTransaction::Custom("Some reason".into())))]);
	assert_watcher_stream!(xt4_watcher, [TransactionStatus::Ready, TransactionStatus::Invalid(TransactionValidityError::Invalid(InvalidTransaction::Custom("Some reason".into())))]);
}
...
fn fatp_watcher_invalid_single_revalidation() {
	...
	assert_eq!(xt0_events, vec![TransactionStatus::Ready, TransactionStatus::Invalid(TransactionValidityError::Invalid(InvalidTransaction::Custom("Some reason".into())))]);
}
...
fn fatp_watcher_invalid_single_revalidation2() {
	...
	let xt0_events = futures::executor::block_on_stream(xt0_watcher).collect::<Vec<_>>();
	debug!(target: LOG_TARGET, ?xt0_events, "xt0_events");
	assert_eq!(xt0_events, vec![TransactionStatus::Invalid(TransactionValidityError::Invalid(InvalidTransaction::Custom("Some reason".into())))]);
	assert_eq!(pool.mempool_len(), (0, 0));
}
...
fn fatp_invalid_tx_is_removed_from_the_pool() {
	...
	assert_watcher_stream!(xt0_watcher, [TransactionStatus::Ready, TransactionStatus::Invalid(TransactionValidityError::Invalid(InvalidTransaction::Custom("Some reason".into())))]);
	assert_watcher_stream!(xt1_watcher, [TransactionStatus::Ready, TransactionStatus::Invalid(TransactionValidityError::Invalid(InvalidTransaction::Custom("Some reason".into())))]);
	assert_watcher_stream!(xt2_watcher, [TransactionStatus::Ready]);
	assert_watcher_stream!(xt3_watcher, [TransactionStatus::Ready]);
}
